 ...
 * 30. TableRow
 * 31. TableCell
 * 32. FootnoteDefinition
 * 33. FootnoteReference
 * 34. TaskItem
 * 35. MultilineBlockQuote
 * 36. Escaped
 * 37. Math
 * 38. WikiLink
 * 39. Underline
 ...
 * 43. Alert
 **/
use super::character_set::character_set;
use super::ctype_custom::isspace;
use super::scanners_custom;
use std::borrow::Cow;
use std::cell::Cell;
...
fn tagfilter(literal: &[u8]) -> bool {
    ...
    let lc = String::from_utf8_lossy(&literal[i..]).to_lowercase();
...
fn dangerous_url(input: &[u8]) -> bool {
    scanners_custom::dangerous_url(input).is_some()
}
    ...
    fn format_node<'a>(&mut self, node: &'a AstNode<'a>, entering: bool) -> io::Result<bool> {
        match node.data.borrow().value {
            ...
            NodeValue::Heading(ref nch) => match self.plugins.render.heading_adapter {
                None => {
                    if entering {
                        ...
                        let mut id = String::from_utf8_lossy(&text_content).to_string();
                        id = self.anchorizer.anchorize(id);

                        let unique_id = if let Some(ref prefix) = self.options.extension.header_ids
                        {
                            format!("{prefix}{id}")
                        } else {
                            id.clone()
                        ...
                        write!(self.output, "<h{} id=\"{}\"", nch.level, unique_id)?;
                        self.render_sourcepos(node)?;
                        self.output.write_all(b">")?;

                        write!(
                            self.output,
                            "<a href=\"#{unique_id}\" class=\"anchor\" title=\"Jump to {id}\"></a>"
                        )?;
                    } else {
            ...
            NodeValue::Link(ref nl) => {
                ...
                // ตรวจสอบว่าไม่ใช่ AutoLink หรืออยู่ในลิงก์ซ้ำ
                        ...
                        ))
                {
                    if entering {
                        ...
                        if nl.url.starts_with("http") {
                            self.output
                                .write_all(b"\" target=\"_blank\" rel=\"noopener noreferrer")?;
                        }
            ...
            NodeValue::Image(ref nl) => {
                if entering {
                    ...
                    // let style_regex = Regex::new(r#"\{style=\"width:(\d+)px;\s*height:(\d+)px;\"\}"#).unwrap();
                    // let width = "300".to_string();
                    // let height = "200".to_string();

                    // if let Some(captures) = style_regex.captures(&nl.url) {
                    //     width = captures[1].sto_string();
                    //     height = captures[2].to_string();
                    // }

                    // ตรวจสอบว่าขนาดของ width และ height ถูกต้องหรือไม่
                    self.output
                        .write_all(format!("\" width=\"{}\" height=\"{}", 1200, 800).as_bytes())?;

                    // ตรวจสอบว่า alt มีค่าหรือไม่
                    self.output.write_all(b"\" alt=\"")?;
                    if !nl.title.is_empty() {
                        self.escape(nl.title.as_bytes())?;
                    } else {
                        self.output.write_all(b"Image")?;
                    }
            ...
            // NOTE: ShortCode feature was removed from comrak
            ...
            NodeValue::Alert(ref alert) => {
                if entering {
                    ...
                    self.output
                        .write_all(format!("{:?}", alert.alert_type).as_bytes())?;
                    self.output.write_all(b"\"")?;
                    self.render_sourcepos(node)?;
                    self.output.write_all(b">\n")?;
                    self.output
                    ...
                    match alert.title {
                        ...
                        None => {
                            self.output.write_all(b"Default Title")?;
                        }
                    }
                    self.output.write_all(b"</p>\n")?;
                } else {
